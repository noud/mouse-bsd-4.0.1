	...
	)
{
	...
	while ((has_mac = rbufp->recv_length - authlen) > MAX_MAC_LEN) {
		...
		case CRYPTO_CERT | CRYPTO_RESP:
			...
			peer->flash &= ~TEST8;
			temp32 = cinfo->nid;
			snprintf(statstr, sizeof(statstr), "cert %s 0x%x %s (%u) fs %u",
		...
		case CRYPTO_IFF | CRYPTO_RESP:
			...
			peer->crypto |= CRYPTO_FLAG_VRFY |
			    CRYPTO_FLAG_PROV;
			peer->flash &= ~TEST8;
			snprintf(statstr, sizeof(statstr), "iff fs %u",
			    ntohl(ep->fstamp));
			record_crypto_stats(&peer->srcadr, statstr);
#ifdef DEBUG
			if (debug)
		...
		case CRYPTO_GQ | CRYPTO_RESP:
			...
			peer->crypto |= CRYPTO_FLAG_VRFY |
			    CRYPTO_FLAG_PROV;
			peer->flash &= ~TEST8;
			snprintf(statstr, sizeof(statstr), "gq fs %u",
			    ntohl(ep->fstamp));
			record_crypto_stats(&peer->srcadr, statstr);
#ifdef DEBUG
			if (debug)
		...
		case CRYPTO_MV | CRYPTO_RESP:
			...
			peer->crypto |= CRYPTO_FLAG_VRFY |
			    CRYPTO_FLAG_PROV;
			peer->flash &= ~TEST8;
			snprintf(statstr, sizeof(statstr), "mv fs %u",
			    ntohl(ep->fstamp));
			record_crypto_stats(&peer->srcadr, statstr);
#ifdef DEBUG
			if (debug)
		...
		case CRYPTO_COOK:
			...
			peer->flash &= ~TEST8;
			snprintf(statstr, sizeof(statstr), "cook %x ts %u fs %u",
		...
		case CRYPTO_SIGN | CRYPTO_RESP:
			...
			peer->flash &= ~TEST8;
			temp32 = cinfo->nid;
			snprintf(statstr, sizeof(statstr), "sign %s 0x%x %s (%u) fs %u",
		...
		case CRYPTO_TAI | CRYPTO_RESP:
			...
			peer->flash &= ~TEST8;
			snprintf(statstr, sizeof(statstr), "leap %u ts %u fs %u", vallen,
	...
	)
{
	...
	if (rval != XEVNT_OK) {
		opcode |= CRYPTO_ERROR;
		snprintf(statstr, sizeof(statstr), "error %x opcode %x", rval, opcode);
...
crypto_update(void)
{
	...
	snprintf(statstr, sizeof(statstr), "update ts %u", ntohl(hostval.tstamp)); 
	...
	)
{
	...
	if ((ptr = strrchr(linkname, '\n')) != NULL)
		*ptr = '\0'; 
	snprintf(statstr, sizeof(statstr), "%s 0x%x len %lu", &linkname[2], ret->flags,
	    len);
	record_crypto_stats(NULL, statstr);
#ifdef DEBUG
	if (debug)
	...
	)
{
	...
	for (j = 0; j < i; j++)
		*ptr++ = htonl(leapsec[j]);
	crypto_flags |= CRYPTO_FLAG_TAI;
	snprintf(statstr, sizeof(statstr), "%s fs %u leap %u len %u", cp, fstamp,
	   leapsec[--j], len);
	record_crypto_stats(NULL, statstr);
#ifdef DEBUG
	if (debug)
