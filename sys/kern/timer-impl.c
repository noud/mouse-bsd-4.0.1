...
/* sys/mbuf.h -> uvm/uvm_param.h -> sys/resourcevar.h
   uses struct rusage, RLIM_NLIMITS
   sys/mbuf.h uses MSIZE
*/
#include <sys/param.h>
...
/* Include this to get __HAVE_TIMECOUNTER when appropriate.
   What it's doing in the arch's types.h, I have no idea. */
...
static void timer_tick(void *); /* forward */
static struct callout timer_callout = CALLOUT_INITIALIZER_SETFUNC(&timer_tick,0);

/* What. The. Fuck.  4.0.1 (a) declares a "min" global in libkern.h,
   *and* (b) warns about shadowing globals! */
#undef min
#define min mmiinn
...
static void timer_tick(void *arg __attribute__((__unused__)))
{
 ...
 if (nrunning > 0)
  { callout_schedule(&timer_callout,1);
  }
 else
  { ticking = 0;
...
static struct protosw timersw[]
 = { { .pr_type = SOCK_STREAM,
       ...
       .pr_ctloutput = &timer_ctloutput,
       .pr_usrreq = &timer_usrreq } };
